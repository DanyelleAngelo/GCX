...
void sort_by_frequency_in_the_round(vector<char*> &ch, int ith, int n_words) {
    ...
    for(int j=0; j < n_words; j++) {
        int pos_in_freq = get_pos(copy[j][ith]);//captura a posição para saber onde devemos olhar
...
void radix_sort(vector<char*> &ch, int rounds) {
    ...
    for(int i=rounds -1; i >= 0; i--) {
        ...
        sort_by_frequency_in_the_round(ch, i, n_words);
    }
}