...
base: compressor.cpp compressor.hpp
	$(CC) -c compressor.cpp -o compressor.o $(addprefix -D, $(MACROS)) $(FLAGS) $(LIBS) 

compressor: base compressor-$(CODEC).cpp compressor-$(CODEC).hpp
	$(CC) -c compressor-$(CODEC).cpp -o compressor-$(CODEC).o $(FLAGS) $(LIBS)
...
ifeq ($(MODE), d)
	...
	cmp $(OUT_PLAIN_TEXT_FILE) $(DIR)/$(FILE) 
else
	./main $(DIR)/$(FILE) $(COMPRESSED_FILE) e $(RULES)
endif
